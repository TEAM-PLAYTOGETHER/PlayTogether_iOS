...
final class EatDetailThunViewController: BaseViewController {
    ...
    private let superViewModel: ThunListViewModel?
    var lightId: Int?
    var imageCount: Int?
    
    init(lightID: Int, superViewModel: ThunListViewModel) {
...
extension EatDetailThunViewController: OneButtonDelegate, TwoButtonDelegate {
    func oneButtonDidTap() {
        guard let originData = try? superViewModel?.eatGoDoThunList.value() else { return }
        let filterData = originData.filter{ $0?.lightID != self.lightId }
        if filterData.isEmpty {
            self.superViewModel?.isEmptyThun.onNext(true)
        } else {
            self.superViewModel?.eatGoDoThunList.onNext(filterData)
        }
        
        superViewModel?.eatGoDoThunList.onNext(originData.filter { $0?.lightID != self.lightId })
